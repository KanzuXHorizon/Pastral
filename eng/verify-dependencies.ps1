...
$agentIpcTree = Invoke-CargoTree -Arguments @('-p', 'pastral-agent', '--edges', 'all')
Assert-ExactPackageVersion -Scope 'resident pastral-agent' -Tree $agentIpcTree -Package 'protobuf' -ExpectedLine 'protobuf v4.35.0-release'
Assert-ExactPackageVersion -Scope 'resident pastral-agent' -Tree $agentIpcTree -Package 'protobuf-codegen' -ExpectedLine 'protobuf-codegen v4.35.0-release'
Assert-ExactPackageVersion -Scope 'resident pastral-agent' -Tree $agentIpcTree -Package 'protobuf-macros' -ExpectedLine 'protobuf-macros v4.35.0-release (proc-macro)'
...
foreach ($package in @('pastral-agent', 'pastral-agent-ipc-probe', 'pastral-ipc-schema', 'pastral-ipc-probe', 'pastral-ipc-transport-probe', 'pastral-ipc-win', 'pastral-manager-ipc-bridge')) {
    $tree = Invoke-CargoTree -Arguments @('-p', $package, '--edges', 'all')
...
Write-Host 'Official protobuf 4.35.0-release is pinned in the resident agent and reviewed IPC/schema/bridge probes; domain/storage/clipboard/agent-core/ipc-auth/ipc-core graphs remain protobuf-free.'
