...
Microsoft's Windows SDK release notes list build 10.0.28000.2270, released in June 2026.
...
- Re-evaluate the pinned patch release during repository bootstrap and before release; upgrading requires build, UI, accessibility, startup, and packaging verification.
...
`AddClipboardFormatListener` registers an HWND for `WM_CLIPBOARDUPDATE`; `GetClipboardSequenceNumber` returns a window-station clipboard serial incremented when clipboard content changes or is emptied.
...
- No polling.
- Sequence number is a coalescing and self-update-suppression signal, not a cryptographic event identifier.
- Delayed rendering semantics mean sequence handling must be covered by fixtures rather than assumed from one notification.
...
- Prefer OLE `IDataObject` enumeration for rich capture and replay.
- Copy data out promptly; never retain a foreign data object longer than required.
...
## 7. Focus-safe overlay primitives
...
## 11. Encryption and key protection
...
- Restrict the pipe to current-user SID, current logon SID, and narrowly justified SYSTEM access.
- Use individual access rights rather than broad `GENERIC_WRITE` where it could permit pipe-instance creation.
- Add protocol versioning, message-size limits, authenticated handshake, request correlation, timeouts, and strict schemas.
