...
- Before paste dispatch, revalidate the intended destination. Cancel rather than paste into an unrelated foreground window.
- Close/cancel returns focus when the prior destination remains valid.
...
- sensitive/private content remains hidden until authorized;
- accessibility tree excludes hidden content.
...
- Warm open target: p95 under 80 ms.
- First results target: p95 under 30 ms at 100,000 ordinary text records on reference hardware.
...
- Destination revalidation prevents wrong-target paste.
...
- Warm open/search budgets are measured, not inferred.
